...
%><head>
	<title><fmt:message key="document" />: <fmt:message key="crud.functionalities.create" /></title>
</head>
<body>
		...
		<table class="create">
			<tbody>
				...
				<c:when test="${not empty document.account}">
					<input type="hidden" name="account" value="<spring:eval expression="document.accountId" javaScriptEscape="true"/>" />
				</c:when>
				<c:otherwise>
				<tr>
