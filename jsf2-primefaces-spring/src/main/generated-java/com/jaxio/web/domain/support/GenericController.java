 ...
 * Template pack-jsf2-spring:src/main/java/domain/support/GenericController.p.vm.java
 */
...
public abstract class GenericController<E extends Identifiable<PK>, PK extends Serializable> {
    ...
    public GenericController(Repository<E, PK> repository) {
    ...
    public List<E> complete(String value) {
        return repository.find(searchParametersForComplete(value));
    }

    protected SearchParameters searchParametersForComplete(String value) {
        return new SearchParameters().anywhere().searchPattern(value);
    }

    /**
     * Ok action is used from sub page (non-readonly) to send the data without saving it. It is expected to be ajax since we want to display the errors, if any,
     * without refreshing the page.
     */
    public String ok() {
        return context().getCallBack().ok(context().getEntity());
    }
    ...
    public String close() {
        ...
        if (!PrimeFacesUtil.showAskForSaveDialog(conversation.getEntityManager(), context().getEntity())) {
    ...
    public String forceClose() {
        return context().getCallBack().notSaved(context().getEntity());
    }
    ...
    public String saveAndClose() {
        if (saveAndCloseInternal(context().getEntity())) {
            return context().getCallBack().saved(context().getEntity());
        }
        return null;
    }
